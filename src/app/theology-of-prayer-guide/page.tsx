...
export default function TheologyOfPrayerGuidePage() {
  const [activeTab, setActiveTab] = useLocalStorage("vine_pray_tab", 0);
  const [openLords, setOpenLords] = useLocalStorage("vine_pray_lords", -1);
  const [openWhy, setOpenWhy] = useLocalStorage("vine_pray_why", -1);
  const [openInt, setOpenInt] = useLocalStorage("vine_pray_int", -1);
  const [journal, setJournal] = useLocalStorage("vine_pray_journal", "");
